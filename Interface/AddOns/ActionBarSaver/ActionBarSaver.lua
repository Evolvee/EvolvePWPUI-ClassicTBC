	...
	for actionID=1, 120 do
		...
		if( type and id ) then
			...
			if( type == "companion" ) then
				set[actionID] = string.format("%s|%s|%s|%s|%s", type, id, "", name, subType)
			-- Save an item
			elseif( type == "item" ) then
				set[actionID] = string.format("%s|%d|%s|%s", type, id, "", (GetItemInfo(id)))
			-- Save a spell
			elseif( type == "spell" and id > 0 ) then
				local spell, rank = GetSpellBookItemName(id, BOOKTYPE_SPELL)
				if( spell ) then
					set[actionID] = string.format("%s|%d|%s|%s|%s", type, id, "", spell, rank or "")
				end
			-- Save a macro
...
function ABS:RestoreMacros(set)
	...
	for id, data in pairs(set) do
		local type, id, binding, macroName, macroIcon, macroData = strsplit("|", data)
		if( type == "macro" ) then
